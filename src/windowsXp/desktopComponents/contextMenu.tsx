...
export function ContextMenu(position: { x: number, y: number, item: string, setOpenWindows: React.Dispatch<React.SetStateAction<WindowData[]>>, appName: string, appIcon: StaticImageData }) {
    ...
    const menuData = position.item === "App" ? appMenuData(position.setOpenWindows, { name: position.appName, icon: position.appIcon }) : position.item === "Desktop" ? desktopMenuData : taskBarMenuData;
        ...
        <div className="bg-white border-[1px] border-gray-400 text-black w-auto fixed" style={{ left: position.x, top: position.y - menuHeight }}>
            {menuData.map((item, index) => (
                <div key={index} className="relative" >
                    <button disabled={item.disabled} onClick={item.function && typeof item.function === 'function' ? () => item.function() : undefined} className={`w-full ${item.disabled ? "text-gray-500" : ""} flex cursor-default px-5 ${item.disabled ? "" : "group hover:bg-winXpBlue"} ${position.item === "Task Bar" ? "bottom-[2000px]" : ""}`} onMouseEnter={() => setActiveIndex(item.functionList ? index : null)} onMouseLeave={() => setActiveIndex(null)}>
