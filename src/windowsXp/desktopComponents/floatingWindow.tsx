...
export default function FloatingWindow({ name, icon, onClose }: FloatingWindowProps) {
    ...
    return (
        <div >
            <div ref={!isMaximized ? targetRef : undefined} id="windowElement" className={`max-w-[screen] !all-unset bg-window p-0 border-x-3 border-b-3 border-winXpBlue !rounded-b-none ${!isMaximized ? "!rounded-t-xl" : "rounded-none"} [&>button]:hidden !flex !flex-col !gap-0 transition-transform duration-0
            ${isMaximized ? "lg:w-[1440px] lg:h-[95vh] lg:absolute w-[100dvw] h-[93.2dvh] top-0 left-0 absolute" : `lg:w-[50vw] lg:h-[600px] lg:fixed  w-[80dvw] h-[50dvh] top-[46.5%] left-[50%] absolute`}`}
                style={!isMaximized ? {
